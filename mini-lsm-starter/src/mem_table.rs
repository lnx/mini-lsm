...
impl MemTable {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) {
        self.map.insert(key.to_vec().into(), value.to_vec().into());
    }
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        let mut iter = MemTableIteratorBuilder {
            ...
            item: (Bytes::from(""), Bytes::from("")),
        }
        .build();
        let item = iter.with_iter_mut(|iter| MemTableIterator::entry_to_item(iter.next()));
        iter.with_mut(|x| *x.item = item);
        iter
    }
...
impl MemTableIterator {
    fn entry_to_item(entry: Option<Entry<'_, Bytes, Bytes>>) -> (Bytes, Bytes) {
        entry
            ...
            .unwrap_or((Bytes::from(""), Bytes::from("")))
    }
}
